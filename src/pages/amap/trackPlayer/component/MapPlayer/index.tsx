...
function EvolutionPlayer(props: MapPlayerProps) {
  ...
  const onPlay = (current: number) => {
    ...
    const nearestTime: string | undefined = timeArray.find(
      (item, index) =>
    ...
    if (nearestTime && nearestTime !== preTime) {
      ...
      if (onFrameChange) {
        onFrameChange(current, nearestTime, data[nearestTime])
      }
    }
  ...
  return (
    <div>
      <Map style={{ width: '100%', height: '80vh' }} {...mapProps}>
        {children({ frameData, preTime } as FrameProps)}
      </Map>
      <Player {...playerProps} onPlay={onPlay} loading={loading} />
    </div>
  )
}
