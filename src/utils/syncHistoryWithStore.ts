...
const defaultSelectLocationState = (state: stateType) => state.routing
...
export default function syncHistoryWithStore(
  history: any,
  store: Store,
  { selectLocationState = defaultSelectLocationState, adjustUrlOnReplay = true } = {},
) {
  ...
  const getLocationInStore = (useInitialIfEmpty?: any) => {
    const locationState: any = selectLocationState(store.getState())
    return (
      locationState.locationBeforeTransitions || (useInitialIfEmpty ? initialLocation : undefined)
    )
  }
